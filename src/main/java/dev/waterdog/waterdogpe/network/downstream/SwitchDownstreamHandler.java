...
public class SwitchDownstreamHandler extends AbstractDownstreamHandler {
    ...
    public final boolean handle(StartGamePacket packet) {
        ...
        if (this.getDownstream().isClosed()) {
            ...
            this.player.setPendingConnection(null);
            this.player.sendToFallback(this.client.getServerInfo(), Color.RED + "Error, contact https://discord.clovercube.net");
            throw CancelSignalException.CANCEL;
        }
        ...
        /*if (newDimension == packet.getDimensionId()) {
            ...
            injectDimensionChange(this.player.getUpstream(), newDimension, packet.getPlayerPosition(),
                    rewriteData.getEntityId(), player.getProtocol());
    ...
    public boolean handle(DisconnectPacket packet) {
        ...
        if (!this.player.sendToFallback(this.client.getServerInfo(), "Transfer Failed"))
        {
            this.player.sendMessage(Color.RED + "Error, contact https://discord.clovercube.net");
        }
